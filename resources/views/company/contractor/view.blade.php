            ...
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('company/contractor') }}" class="pjax">Contractor</a></li>
...
<div class="row">
  <div class="col-12 order-1 order-md-0">
    <div class="card mb-4">
      <div class="card-body d-flex flex-column flex-sm-row gap-0 gap-sm-3 gap-lg-2 gap-xl-4">
       ...
       <div class="commentForm">
            <form id="commentForm" action="{{ route('company/contractor/comment-save') }}" method="post">
                @csrf
                <input type="hidden" name="contractor_id" value="{{@$model->id}}">
                <div class="form-group">
                    <label class="form-label" for="basic-icon-default-fullname">Note</label>
                    <textarea class="form-control " rows="3" name="contractor_internal_note" placeholder="" id="comment">{{@$model->contractor_internal_note}}</textarea>
               </div>
               <div class="form-group">
                    <label class="form-label" for="basic-icon-default-fullname">Phone</label>
                    <input  type="tel" class="form-control"  name="phone"  id="phone" value="{{ @$model->phone }}" pattern="^\+?[0-9]+$" inputmode="numeric" oninput="this.value = this.value.replace(/[^0-9+]/g, '')" />
               </div>
                <div class="form-group">
                    <label class="form-label" for="basic-icon-default-fullname">Address</label>
                    <textarea class="form-control " rows="2" name="address" placeholder="" id="address">{{@$model->address}}</textarea>
               </div>
        ...
        <div class="col-lg-6">
            <div class="card equal-cards">
                <div class="card-header d-flex flex-wrap gap-2 align-items-center justify-content-between">
                    ...
                    @if($extension == 'pdf')
                    <div class="">
                        <button class="btn btn-primary" onclick="app.showModalView('{{ route('company/contractor/document-file/pdf-preview',['fileName'=> $userAccountModel->file_name]) }}')">Preview</button>
                        <a  href="{{ route('company/document/show', ['fileName' => $userAccountModel->file_name]) }}" class="btn btn-success" download>Download</a>
                    </div>
                    @endif
                </div>
                <div class="card-body">
                         ...
                         @if($extension !== 'pdf')
                          <div class="col-12 mt-2 mt-md-0 mb-md-0 mb-2">
                               <div class="view-info-img">
                                   <img class="w-100 active landscap-img" src=" {{ route('company/document/show', ['fileName' => $userAccountModel->file_name]) }}" data-bs-target="#Gallerycarousel" data-bs-slide-to="0">
                               </div>
                          </div>
                          @endif
                    </div>
...
documentReady(function() {
    ...
    datatableObj = $('#data-table').DataTable({
         ...
         stateSave :true,
      ajax: {
        url: "{{route('company/contractor/document',['id'=>$model->id])}}",
        method: 'post',
        dataSrc: 'data',
       data: function(d) {
...
documentReady(function() {
    datatableObj1 = $('#data-table1').DataTable({
         ...
         stateSave :true,
      ajax: {
        url: '{{route("company/contractor/documents-activity" , ['id'=>@$model->id])}}',
        method: 'post',
        dataSrc: 'data',
        data: {
          '_token': CSRF_TOKEN
