...
<?php $fromCame = isset($_GET['fromCame']) ? $_GET['fromCame'] : '' ?>
    ...
    <div class="card">
      <div class="card-body">
        ...
        <form class="ajax-form" action="{{route('auth/verify-process')}}" method="post">
          ...
          <input type="hidden" name='fromCame' value ="{{@$fromCame}}">
...
<script type="text/javascript">
  ...
  function resendOtp(){
    app.ajaxPost('{{ route('auth/resend-otp') }}',{type:'{{$type}}',code:'{{@$code}}',token:'{{@$token}}',fromcame:'{{@$fromCame}}'})
  }
</script>
@endpush
