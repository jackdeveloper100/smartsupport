...
<div class="row main-log-pages">
    <div class="col-lg-5 col-12">
        <div id="auth-left">
            ...
            <form id="ajax-form1" class="mb-3" action="{{ route('admin/auth/login-process') }}" method="POST">
                {{ csrf_field() }}
            ...
            <div class="error-msg bottom-error-msg" style="display:none;">
                ...
                <button class="btn btn-primary btn-block btn-lg shadow-lg mt-4 mt-sm-3" type="submit">Log in</button>
            </form>

            <div class="text-center mt-2 mt-sm-2 text-lg fs-5">
                <p><a class="font-bold" href="admin/auth/password-forgot">Forgot password?</a>.</p>
            </div>
        </div>
    </div>
...
documentReady(function() {
  ...
  $('#ajax-form1').validate({
    ...
    invalidHandler: function () {
      ...
      $submitBtn.prop('disabled', true);  // Disable button immediately
      ...
      $.ajax({
        ...
        success: function(response) {
          if (response.status === 1) {
            ...
            $('.error-msg').show();
            $submitBtn.prop('disabled', false);  // Re-enable button on failure
          }
        },
        error: function(xhr, status, error) {
          $('#error-message').html('An error occurred. Please try again.');
          $('.error-msg').show();
          $submitBtn.prop('disabled', false);  // Re-enable button on error
        }
      });
