        ...
        <div class="card">
            <div class="card-header justify-content-between d-flex align-items-center flex-wrap gap-2 pb-3">
                ...
                <div class="d-flex flex-wrap flex-wrap gap-2 align-items-end flex-wrap">
                    <!--@if($sessionUser->hasPermission('admin/contractor/export-data'))-->
                    <!--  <a class="btn btn-success d-none" href="#" id="exportData" type="button" data-id="">Export Data</a>-->
                    <!--@endif-->
                    @if($sessionUser->hasPermission('admin/contractor/send-remindermail'))
                      <a href="javascript:void(0)" class="btn btn-secondary d-none" type="button" id="sendReminderMail">Send Reminder Email</a>
                    @endif
                    @if($sessionUser->hasPermission('admin/contractor/delete_multiple'))
                      <button class="btn btn-danger d-none" type="button" onclick="app.confirmAction(this);" data-action="admin/contractor/delete_multiple" data-id="' . selected_id[] . '"  id="deleteEntreeBtn">Delete Selected</button>
                    @endif
                    @if($sessionUser->hasPermission('admin/contractor/create'))
                    <a href="admin/contractor/create" class="btn btn-primary d-sm-inline-block  pjax" style="float: inline-end;">Create</a>
                    @endif
                ...
                <div class="form-group mb-0">
                        ...
                        <select class="form-select"
                           name="status" onchange="tableFilterExpire();" id="company_name">
                            <option value="">All</option>
                    ...
                    <table class="datatable-list-table table border-top" id="data-table">
                        <thead>
                            ...
                            <th>Last Activity Date</th>
                             @if($sessionUser->hasPermission('admin/contractor/view') || $sessionUser->hasPermission('admin/contractor/update') || $sessionUser->hasPermission('admin/contractor/delete'))
                              <th>Actions</th>
                              @endif
                            </tr>
                        </thead>
...
function tableFilterExpire(){
    ...
    var tableDataExp = {
        ...
        $sessionUser->hasPermission('admin/contractor/view') ||
        $sessionUser->hasPermission('admin/contractor/update') ||
        $sessionUser->hasPermission('admin/contractor/delete')
    ) !!};
    datatableObj = $('#data-table').DataTable({
         ...
         lengthMenu: [25, 50, 100],
          stateSave :true,
        ajax: {
            url: '{{ route("admin/contractor/list") }}',
            method: 'post',
            dataSrc: 'data',
            data: function (d) {
        ...
        columns: [
            ...
            { data: "id", title: "Id", responsivePriority: 4 }, // Serial No.
            { data: "first_name", responsivePriority: 4 },
            { data: "business_name", responsivePriority: 2 },
            { data: "company_id", responsivePriority: 2 },
...
function updateExportLink(selectedIds) {
    ...
    $('#exportData').attr('href', '{{ route('admin/contractor/export-data', ['id' => 'REPLACE_ID']) }}'.replace('REPLACE_ID', ids));
    $('#exportData').removeClass('d-none');
}
 ...
 function updateReminderLink(selectedIds) {
        ...
        const url = '{{ route('admin/contractor/send-remindermail', ['id' => 'REPLACE_ID']) }}'.replace('REPLACE_ID', ids);
