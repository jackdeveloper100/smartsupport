...
<form class="ajax-form" method="post" action="{{ route('admin/contractor/save') }}" enctype="multipart/form-data" id="ajax-form">
    @csrf
                ...
                <div class="col-lg-6 col-md-12 col-sm-6 col-12">
                     <div class="form-group ">
                         <label class="form-label" for="basic-icon-default-fullname">Company Name <span class="star">*</span></label>   
                            <select class="choices form-select" name="company_name">
                                <option value="">Select Company</option>
                            @foreach($companyName as $company)
                                <option value="{{ $company->id }}" @if(isset($model->company_id) && $model->company_id == $company->id) selected @endif>{{ $company->company_name }}</option>
                            @endforeach
                            </select>
                        </div>
                </div>
  ...
  <a href="admin/contractors" class="btn btn-secondary" style="color: white;">Back</a>
</form>
