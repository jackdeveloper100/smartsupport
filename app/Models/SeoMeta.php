    ...
    public function Seometalist(array $postData): array
    {
        ...
        foreach ($result['data'] as $row) {
            ...
            if ($sessionUser->hasPermission('admin/seo/update')) {
                $row->action .= sprintf(
                    '<a href="admin/seo/update?id=%d" class="text-body pjax" title="Update"><i class="fa fa-edit me-2"></i></a>&nbsp;',
                    $row->id
                );
            }
            ...
            if ($sessionUser->hasPermission('admin/seo/delete')) {
                $row->action .= sprintf(
                    '<button style="border:none; background:none;" onclick="app.confirmAction(this);" data-action="admin/seo/delete?id=%d"  class="text-body" title="Delete"><i class="fa fa-trash mx-2"></i></button>',
                    $row->id
                );
            }
        }
